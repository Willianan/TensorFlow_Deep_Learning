...
"""
作   者：Charles Van
创建时间：2019/1/23 16:35
文   件：9-23_yuyinchall.py
...
#自定义
yuinutils= __import__("9-24_yuyinutils.py")
sparse_tuple_to_texts_ch = yuinutils.sparse_tuple_to_texts_ch
ndarray_to_text_ch = yuinutils.ndarray_to_text_ch
get_audio_and_transcriptch = yuinutils.get_audio_and_transcriptch
pad_sequences = yuinutils.pad_sequences
sparse_tuple_from = yuinutils.sparse_tuple_from
get_wavs_labels = yuinutils.get_wavs_labels
...
def BiRNN_model(batch_x,seq_length,n_input,n_context,n_character,keep_dropout):
	...
	with tf.name_scope('fc1'):
		...
		layer_2 = tf.minimum(tf.nn.relu(tf.add(tf.matmul(layer_2,h2),b2)),relu_clip)
		layer_2 = tf.nn.dropout(layer_2,keep_dropout)
	
	#3rd layer
	with tf.name_scope('fc3'):
		b3 = variable_on_cpu('b2',[n_hidden_3],tf.random_normal_initializer(stddev = b_stddev))
		h3 = variable_on_cpu('h2',[n_hidden_2,n_hidden_3],tf.random_normal_initializer(stddev = h_stddev))
		layer_3 = tf.minimum(tf.nn.relu(tf.add(tf.matmul(layer_2,h3),b3)),relu_clip)
		
	#双向RNN
	with tf.name_scope('lstm'):
		#Forward direction cell
		lstm_fw_cell = tf.contrib.BasicLSTMCell(n_cell_dim,forget_bias = 1.0,state_is_tuple =True)
		lstm_fw_cell = tf.contrib.DropoutWrapper(lstm_fw_cell,input_keep_prob = keep_dropout)
		#Backward direction cell
		lstm_bw_cell = tf.contrib.BasicLSTMCell(n_cell_dim,forget_bias = 1.0,state_is_tuple = True)
		lstm_bw_cell = tf.contrib.DropoutWrapper(lstm_bw_cell,input_keep_prob = keep_dropout)
		#'layer_3' '[n_steps,batch_size,2*n_cell_dim]'
		layer_3 = tf.reshape(layer_3,[-1,batch_x_shape[0],n_hidden_3])
		outputs,outpt_states = tf.nn.bidirectional_dynamic_rnn(cell_fw=lstm_fw_cell,cell_bw=lstm_bw_cell,inputs=layer_3,\
...
wav_path = 'data_thchs30/data_thchs30/train'
label_file = 'data_thchs30/doc/trans/train.word.txt'
