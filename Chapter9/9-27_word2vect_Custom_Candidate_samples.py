...
@time：2019/1/26 13:46
@Project:TensorFlow_Deeping_learning
@Filename:9-27_word2vect_Custom_Candidate_samples.py
"""
...
training_file = 'wordtest_data/70test.txt'
...
#分词
def classed_words(traing_data):
	seg_list = jieba.cut(traing_data)
	training_word = " ".join(seg_list)
	training_word = training_word.split()
	#以空格将字符串分开
	training_word = np.array(training_word)
...
training_word = classed_words(training_data)
print("总词数：",len(training_word))
training_label,count,dictionary,words,vocab_freqs = build_dataset(training_word,350)
...
for i in range(8):              #取第一个字，后一个的是标签，再取其前一个字当标签
...
num_skips = 2                               #How many times to an input to generate a label.

valid_size = 16                             #Random set of words to evaluate similarity on.
valid_window = np.int32(words_size/2)       #Only pick dev samples in the head of the head of the distribution.
...
train_inputs = tf.placeholder(tf.int32,shape = [bath_size])
train_labels =tf.placeholder(tf.int32,shape = [bath_size,1])
...
sampled = tf.nn.fixed_unigram_candidate_sampler(true_classes = tf.cast(train_labels,tf.int64),num_true=1,\
                                                num_sampled=num_sampled,unique=True,range_max=words_size,\
...
loss = tf.reduce_mean(tf.nn.sampled_softmax_loss(weights=nce_weights,biases=nce_biases,labels=train_labels,\
                                                 inputs=embed,num_sampled=num_sampled,num_classes=words_size,\
...
with tf.Session() as sess:
	...
	for step in range(num_steps):
		batch_inputs,batch_labels = generate_batch(training_label,bath_size,num_skips,skip_window)
		feed_dict = {train_inputs:batch_inputs,train_labels:batch_labels}

		#We perform one update step by evalating the optimizer op (including it in the list of returned values for session.run())
		...
		if step % 2000 == 0:
			if step > 0 :
				average_loss /= 2000
			#The avwerage loss is an astimate of the loss over the last 2000 barchea
		...
		if step % 10000 == 0:
			...
			for i in range(valid_size):
				...
				top_k = 8    #number of nearset neighbors
