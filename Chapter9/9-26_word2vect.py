...
@time：2019/1/24 20:03
@Project:TensorFlow_Deeping_learning
@Filename:9-26_word2vect.py
"""
...
#分词
def Classing_words(training_data):
...
training_word = Classing_words(training_data)
print("总词数：",len(training_word))
training_label,count,dictionary,words = build_dataset(training_word,350)
...
batch_size = 128
embedding_size = 128     #Dimension of the enbedding vector.
...
with tf.device('/cpu:0'):
	...
	embeddings = tf.Variable(tf.random_uniform([words_size,embedding_size],-1.0,1.0))#94个，每个128向量
...
loss = tf.reduce_mean(tf.nn.nce_loss(weights=nce_weights,biases=nce_biases,labels=training_labels,inputs=embed,num_sampled=num_sampled,num_classes=words_size))

#Compute the SGD optimizer using a learning rate of 1.0
...
with tf.Session() as sess:
	...
	for step in range(num_steps):
		...
		feed_dict = {training_inputs:batch_inputs,training_labels:batch_labels}

		#We perform one update step by evaluating optimizer  op(including it
		#in the list of returned values for session.run())
		_,loss_val = sess.run([optimizer,loss],feed_dict = feed_dict)
		average_loss += loss_val

		#通过打印测试可以看到embed的值在逐渐的被调节
		# emv = sess.run(embed,feed_dict = {training_inputs:[37,18]})
		#print("emv--------------------",emv[0])
