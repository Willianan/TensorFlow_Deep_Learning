...
@time：2019/1/29 10:45
@Project:TensorFlow_Deeping_learning
@Filename:10-9_Conditional_variational_self-encoding.py
"""
...
with tf.Session() as sess:
	...
	# 根据图片模拟生成图片
	show_num = 10
	pred = sess.run(reconstruction, feed_dict={x: mnist.test.images[:show_num], y: mnist.test.labels[:show_num]})
	...
	pred = sess.run(reconstructionout, feed_dict={zinput: z_sample, y: mnist.test.labels[:show_num]})
	f, a = plt.subplots(2, 10, figsize=(10, 2))
	for i in range(show_num):
		a[0][i].imshow(np.reshape(mnist.test.images[i], (28, 28)))
		a[1][i].imshow(np.reshape(pred[i], (28, 28)))
