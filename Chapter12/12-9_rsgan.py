...
def Discriminator(dbatch, name="Discriminator"):
	with tf.variable_scope(name):
		...
		for i in range(7):
			net = slim.conv2d(net, ochannels[i], 3, stride=stride[i % 2], activation_fn=leaky_relu,
			                  normalizer_fn=slim.batch_norm, scope='block' + str(i))
...
with tf.Session() as sess:
	...
	try:
		def train(endpoint, gen_step, disc_step):
			...
			while global_step.eval() <= endpoint:
				...
				if ((global_step.eval() / 2) % log_steps == 0):  # 一次走两步
					...
					s = time.strftime('%Y-%m-%d %H:%M:%S:', time.localtime(time.time())) + 'step=' + str(
						global_step.eval()) + ' mse=' + str(mse) + ' psnr=' + str(psnr) + ' ssim=' + str(
						ssim) + ' gen_loss=' + str(gen_loss.eval()) + ' disc_loss=' + str(disc_loss.eval())
					print(s)
		...
		###显示
		resultv, imagesv, x_smallv, x_nearestv, x_bilinv, x_bicubicv, y_predv = sess.run(
			[result, images, x_smalls, x_nearests, x_bilins, x_bicubics, y_pred])
